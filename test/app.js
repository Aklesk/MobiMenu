...
describe('Main App', () => {
    ...
    describe('addRecord function tests', () => {

        it("ran addRecord on menu, category, and product records", () => {
        ...
        it("added the newly created record to recordDict and dataObj state", () => {
            expect(typeof(app.state.recordDict["menutest"])).to.not.equal('undefined')
            expect(app.state.dataObj.menus.filter((m) => {
                return m.guid == "menutest"
            })).to.have.length(1)
            expect(typeof(app.state.recordDict["categorytest"])).to.not.equal('undefined')
            expect(app.state.dataObj.categories.filter((m) => {
                return m.guid == "categorytest"
            })).to.have.length(1)
            expect(typeof(app.state.recordDict["producttest"])).to.not.equal('undefined')
            expect(app.state.dataObj.products.filter((m) => {
                return m.guid == "producttest"
            })).to.have.length(1)
        })

        it("added newRec flag to all newly created records", () => {
    ...
    describe('deleteRecord function tests', () => {

        it("ran a delete operation on all previously created records", () => {
        ...
        it("properly removed the records from recordDict and dataObj state", () => {
            expect(typeof(app.state.recordDict["menutest"])).to.equal('undefined')
            expect(app.state.dataObj.menus.filter(m => m.guid == "menutest")).to.be.empty
            expect(typeof(app.state.recordDict["categorytest"])).to.equal('undefined')
            expect(app.state.dataObj.categories.filter(m => m.guid == "categorytest")).to.be.empty
            expect(typeof(app.state.recordDict["producttest"])).to.equal('undefined')
            expect(app.state.dataObj.products.filter(m => m.guid == "producttest")).to.be.empty
        })
    })
    ...
    describe('editing state tests', () => {
        ...
        it("returned correct blank editing state", () => {
            expect(app.editing()).to.deep.equal({rec: "", elem: "", saveFunc: null})
        })

        it("set an edit state", () => {
        ...
        it("changed edit state via editing function, saving previous edit in the process", () => {
    ...
    describe('onClick function tests', () => {

        it("completed an onClick function with no edit state", () => {
            expect(app.state.editing).to.deep.equal({rec: "", elem: "", saveFunc: null})
            expect(app.onClick()).to.equal(undefined)
        })

        it("completed an onClick function with an active edit state", () => {
            expect(app.editing(
                "testRecord",
                "element",
                (r) => {
        ...
        it("reset editing state after onClick", () => {
            expect(app.editing()).to.deep.equal({rec: "", elem: "", saveFunc: null})
        })

        it("saved edited record to state after onClick was run", () => {
    ...
    describe('overlay tests', () => {
        ...
        it("does not contain an any active overlay elements", () => {
            wrapper.update()
        ...
        it("set and displayed an Alert overlay", () => {
            expect(app.overlay(
                    "header",
                    "message",
                    "alert",
        ...
        it("set and displayed a Question overlay", () => {
            expect(app.overlay(
                    "header?",
                    "message?",
                    "question",
        ...
        it("set and displayed an Add Product overlay", () => {
            expect(app.overlay(
                    "header",
                    "message",
                    "addProduct",
        ...
        it("set and displayed an Add Category overlay", () => {
            expect(app.overlay(
                    "header",
                    "message",
                    "addCategory",
        ...
        it("cleared overlay state with okayFunc, calling overlay saveFunc once in the process", () => {
    ...
    describe('record tests', () => {
        ...
        it("correctly threw out updateRecord input which did not already exist in state", () => {
        ...
        it("Sent an existant record update to state with updateRecord function", () => {
        ...
        it("stripped off the newRec key during record update with updateRecord", () => {
        ...
        it("correctly threw out updateList input which did not already exist in state", () => {
            expect(app.updateList({guid: "blargh", intName: "blargh"})).to.equal(undefined)
            expect(app.state.recordDict["blargh"]).to.equal(undefined)
        })

        it("updated existant record with updateList command", () => {
